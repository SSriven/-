...
namespace PublicLibrary.加解密
{
    public class MD5Class
    {
        ...
        /// <returns>MD5-32位小写</returns>
        public string SignatureFormatter(string Str)
        {
            return Md5Encrypt(Str).ToLower();
        }
        /// <summary>
        /// 签名验证
        /// </summary>
        ...
        /// <returns></returns>
        public bool CheckSignature(string str,string signstr)
        {
            if (Md5Encrypt(str).ToLower() == signstr.ToLower())
            {
                return true;
            }
            else return false;
        ...
        #region MD5加密
        public string Md5Encrypt(byte[] sourceBytes)
        {
            ...
            int startIndex = 0;
            while (startIndex <= sourceBytes.Length - 64)
            {
                GetHashBlock(sourceBytes, ref abcd, startIndex);
                startIndex += 64;
            }

            var md5Bytes = GetHashFinalBlock(sourceBytes, startIndex, sourceBytes.Length - startIndex, abcd, (long)sourceBytes.Length * 8);
        ...
        public string Md5Encrypt(string source)
        {
            ...
            var md5Bytes = GetHashFinalBlock(sourceBytes, startIndex, sourceBytes.Length - startIndex, abcd, (long)sourceBytes.Length * 8);
            return BytesToHexString(md5Bytes).ToLower();
        }

        /// <summary>
        /// md5字符串加密
        /// </summary>
        public byte[] Md5EncryptAsBytes(string source)
        {
            var encoding = new UTF8Encoding();
            var sourceBytes = encoding.GetBytes(source);
            ...
            return GetHashFinalBlock(sourceBytes, startIndex, sourceBytes.Length - startIndex, abcd, (long)sourceBytes.Length * 8);
        }
        ...
        private byte[] GetHashFinalBlock(byte[] input, int ibStart, int cbSize, AbcdStruct abcd, long len)
        {
        ...
        private void GetHashBlock(byte[] input, ref AbcdStruct abcdValue, int ibStart)
        {
